...
- Faithful Re-host / Webflow / WebGL: `rehost/` (serves real assets and pristine JS)
- Authenticated SPA (with Local Proxy): `scripts/local-api-proxy.js` + `rehost/` bundle (serves actual frontend and intercepts API requests locally).

## DELIVERABLE MODE CHECK (FIRST STEP)

Before writing any code, check `DELIVERABLE` in `context.md`:
1. If `DELIVERABLE=faithful_rehost`: DO NOT attempt to rewrite the site in React. Follow `references/faithful-rehost.md`, serve the mirrored assets directly, decompress any gzipped CSS/JS, and ensure engine scripts (`spacers.js`, `webflow.js`, Three.js) are byte-for-byte pristine.
2. If `DELIVERABLE=fully_functional_copy`: Follow `references/local-api-proxy.md`, rehost the frontend bundle, and launch `local-api-proxy.js` with synthetic mocks so login and app navigation function offline.
3. If `DELIVERABLE=simulated_clone` or `react_impl`: Proceed with the React/Tailwind component implementation below.

## THE ANTI-AI-SLOP DOCTRINE (MANDATORY)

"AI slop" refers to generic, hallucinated, or lazy approximations. The cloner MUST obey these rules:

1. **ZERO TEXT HALLUCINATION**:
   - Every single headline, subheadline, paragraph, badge, button label, pricing dollar amount, feature bullet, and footer link MUST be copied VERBATIM from `copy.md`, `context.md`, or the live DOM.
   - NEVER invent generic filler (e.g. "In today's fast-paced world", "Transform your business with cutting-edge solutions", "Lorem ipsum").
2. **ZERO TRUNCATION (NO LAZY CODING)**:
   - If a grid or carousel has 12 poses or 48 items, render all 12 or 48 items.
   - NEVER truncate lists to 2 or 3 items and leave comments like `{/* remaining items */}`.
3. **NO EMOJIS FOR ICONS**:
   - Never use emoji unicode characters (🚀, 📱, 🔍, 💡, ⭐️) for UI elements.
   - Use the exact inline SVG paths extracted from the site or from `public/icons/`.
4. **ALL DROPDOWNS & MENUS MUST FUNCTION**:
   - Every dropdown menu, mobile hamburger drawer, and modal captured in `screenshots/` must be fully interactive with state toggles (`useState`) and match the original item list.
5. **EXACT COMPUTED TOKENS**:
   - Use exact hex colors, border-radii, and spacing values from `context.md`. Never guess Tailwind default colors like `bg-blue-600` when the original was `#2563eb`.
