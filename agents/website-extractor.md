...
- For inline `<svg>` elements in the DOM: extract their outerHTML verbatim, preserving viewBox, paths, and fill/stroke attributes. Save them as `public/icons/{name}.svg` or document them in `context.md`.
...
## Verbatim Text & DOM Content Extraction (CRITICAL: Anti-AI-Slop)

**NEVER allow the cloner agent to guess or write AI filler text.** Extract every single character of copy verbatim from the live page:

1. **Extract Complete Copy Tree**:
   - Run a DOM extraction script to capture all headings (`h1`, `h2`, `h3`, `h4`), paragraphs (`p`), lead-ins, badge texts, button labels, chip texts, pricing numbers, disclaimers, FAQ questions/answers, and footer column links.
   - Save this verbatim text to `${TASK_DIR}/copy.md` or `${TASK_DIR}/content.json`.
2. **List & Table Item Parity**:
   - Count and record every item in feature grids, testimonial carousels, logos marquee, pose cards, or comparison tables.
   - Example: if a section contains 12 poses or 48 preview cards, record all 12 or 48 items with their exact titles, subtitles, and image links.
3. **Internal Links Inventory**:
   - Record every navigation link and footer link (`text` -> `href`) so all internal routes are mapped without broken `#` links.
...
- ZERO AI SLOP: Never summarize copy, never replace text with "lorem ipsum", and never truncate lists of items. Extract the full text content and write it to copy.md.
- RAW SVG CAPTURE: Always preserve exact SVG paths rather than substituting generic icons.
