...
### 🔴 Critical (Must Fix Before Delivery - Immediate Reject)
- **AI Slop / Hallucinated Text**: Any occurrence of placeholder copy ("Lorem ipsum", "in today's fast-paced world", "elevate your experience", "transformative solution") or copy that deviates from the original live site.
- **Lazy Truncation**: A list or grid that renders fewer items than the live site (e.g. 3 cards rendered when live has 12).
- **Emoji As UI Icon**: Using emoji characters (🚀, 📱, 🔍) instead of SVG vector icons.
- **Console Errors or 404s**: Any broken image, missing font, failed WASM/model load, or JavaScript runtime error.
- **Missing Chunks or Dynamic Module Failures**: Any runtime `TypeError: Failed to fetch dynamically imported module` or missing code-split chunks.
- **Error Boundaries / Subroute Crashes**: Any `"Uh-oh! Something went wrong"` screen when navigating to action subroutes (e.g. `/campaigns/new`, `/agents/create`, step wizards, modal dialogs).
- **Missing sections, menus, or dropdowns**: Missing header dropdown items, missing mobile hamburger menu, or skipped footer links.
- **Broken Master-Detail Click Interactions**: Clicking table rows, conversation threads, or list items fails to open the detail panel/drawer or displays empty data.
- **Completely wrong colors or fonts**: Using generic Tailwind defaults instead of extracted hex values.
- **Broken layout or severe misalignment**: Overflow, broken sticky positioning, or non-functional side-scroll/parallax.
...
1. Start the dev server or static server and confirm the clone is accessible
2. Run the automated 1-to-1 visual & code verification engine:
   ```bash
   node scripts/verify-1to1.js --live <liveBase> --local <localBase> --routes <routes.json> --out comp/verify
   ```
3. Inspect `comp/verify/verify-report.md` for:
   - Visual pixel match percentage (target >= 95%)
   - Element count parity (images, SVGs, buttons, headings, links)
   - Computed CSS style parity (colors, fonts, widths)
4. Open `comp/verify/compare-viewer.html` to visually drag the split slider across every screen and inspect the neon magenta diff overlays.
5. Compile all findings into `review-notes.md` with priority classifications.
