...
## Why the Anti-AI-Slop & Verbatim Copy Rule?

### The Problem with AI Hallucinated Copy
When AI agents lack the original text, they invent placeholder marketing phrases ("In today's fast-paced world", "Elevate your business", "Seamlessly integrate cutting-edge tools", "Lorem ipsum"). This immediately ruins the credibility of a clone and makes it look like generic AI slop.

### The Solution: Verbatim DOM Text Harvesting
1. The extractor extracts all text nodes from the live page into `copy.md`.
2. The cloner copies headings, paragraphs, and labels word-for-word.
3. The QA reviewer fails any build that contains AI buzzwords or altered copy.

---

## Why Faithful Re-Host for Complex Motion, WebGL & SPAs?

### The "React Approximation Trap"
Attempting to rewrite bespoke animation engines (Webflow IX2, skrollr, GSAP ScrollTrigger, Three.js canvas, PlayCanvas) into a hand-rolled React component fails because:
1. Complex engines rely on hundreds of layered elements, custom wheel math, or binary WebGL decoders (`.ktx2`, `.glb`, `.wasm`).
2. Hand-rolled React approximations flatten horizontal side-scrolls, lose 3D models, or break intersection observers.

### The Solution: Faithful Re-Host + Local API Proxy
- For motion, canvas, and Webflow sites: serve the site's own HTML/CSS/JS files directly, keep engine JS byte-for-byte pristine, and decompress gzipped CDN files.
- For authenticated SPAs: re-host the real frontend bundle and use `scripts/local-api-proxy.js` with `mocks.json` to handle offline auth and mock API data.
